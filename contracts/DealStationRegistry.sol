...
contract DealStationRegistry is IERC6551Registry, Context {
  ...
  mapping(address => uint) public numberOfAccountCreated;
  ...
  modifier onlyWhenOwnerHaveTicket() {
    uint256 balance = nftContract.balanceOf(_msgSender()) -
      numberOfAccountCreated[_msgSender()];
    require(
      balance > 0,
      "DealStationRegistry: You have already used your tickets"
    );
    _;
  }
  ...
  ) public onlyWhenOwnerHaveTicket returns (address) {
    numberOfAccountCreated[_msgSender()]++;
